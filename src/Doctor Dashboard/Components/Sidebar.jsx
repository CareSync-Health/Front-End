...
const Sidebar = () => {
  ...
  return (
    <div className={`bg-[#fff] h-screen pt-4 ${open ? "w-[15rem]" : "lg:w-20 xs:w-12"} duration-300 relative`}>
        <BsArrowLeftShort 
        className={`bg-white text-[#22D1EE] text-2xl rounded-full absolute -right-3 top-9 border border-[#22D1EE] cursor-pointer lg:flex xs:hidden ${!open && "rotate-180 text-[20px]"}`}
      ...
      <ul className='pt-2'>
            {Menus.map((menu, index) => (
              <>
                  <li key={index} className={`text-${activeIndex === index ? '[#22D1EE]' : '[#000]'} font-Lato font-bold flex items-center gap-x-0 cursor-pointer p-2 hover:bg-light-white mt-2  ${menu.spacing ? "mt-6 border-t pt-[2rem] rounded-0" : "mt-2"}`} onClick={() => handleMenuClick(index, menu.Link)}>
